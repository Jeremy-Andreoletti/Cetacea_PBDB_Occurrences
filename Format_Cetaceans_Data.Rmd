...
# Check that all the datasets contain the same taxa names
...
```{r }
# Get the stratigraphic age uncertainty interval
cetacea_pbdb$age_uncertainty_interval <- mapply(function(min_age, max_age){paste("[",-max_age,", ",-min_age,"]", sep = "")}, cetacea_pbdb$min_ma, cetacea_pbdb$max_ma)

# Compute the mean age in the interval
cetacea_pbdb$age_mean <- (cetacea_pbdb$min_ma+cetacea_pbdb$max_ma)/2

ggplot(cetacea_pbdb, aes(x=-max_ma, y=age_uncertainty_interval)) +
  geom_segment(aes(x = -max_ma, y = reorder(age_uncertainty_interval, -age_mean), xend = -min_ma, yend=reorder(age_uncertainty_interval, -age_mean), color=-age_mean)) +
...
First, one can use the stratigraphic age uncertainty interval midpoint as the occurrence fixed age.
...
ggplot(cetacea_pbdb, aes(x=-age_mean, y=rnorm(length(age_mean), 1, 0.05))) +
  ...
  theme(panel.background = element_blank(),
        ...
        axis.ticks.y = element_blank()) +
  ggtitle(paste("Repartition of", dim(cetacea_pbdb)[1], "recorded occurrences through time - Midpoint"))
...
$\to$ Numerous occurrences seem to have the same stratigraphic age uncertainty so in order to avoid clusters let's draw them uniformly in their interval rather than taking the midpoint.

```{r, fig.asp=0.3, fig.width=8}
# Draw occurrence age uniformally in the uncertainty interval
...
ggplot(cetacea_pbdb, aes(x=-age_runif, y=rnorm(length(age_runif), 1, 0.05))) +
  ...
  theme(panel.background = element_blank(),
        ...
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank()) +
  ggtitle(paste("Repartition of", dim(cetacea_pbdb)[1], "recorded occurrences through time - Uniform draw"))
...
$\to$ The distribution seems much smoother now.

### Uniform subsampling

These occurrences are too numerous to perform a quick preliminary analysis, so let's randomly subsample a fraction of them.
...
ggplot(cetacea_pbdb_subsampled, aes(x=-age_runif, y=rnorm(length(age_runif), 1, 0.05))) +
  ...
  theme(panel.background = element_blank(),
        ...
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank()) +
  ggtitle(paste("Repartition of", dim(cetacea_pbdb_subsampled)[1], "recorded occurrences through time - Uniform draw"))
...
Occurrences are associated with an **accepted rank**, depending on the level of detail of their description and attribution to a given taxonomic position.
...
$\to$ Half of the occurrences are identified at the level of the species and 1/3 at the genus or family.
...
$\to$ An apparent huge cluster of occurrences appears in recent times, with very precise dating = Artifact due to the "Pull of the Recent" effect ? Bones collected by ancient hominids ?

$\implies$ These occurrences seem to obey to a completely different process and over a very short period of time, so we decided to **remove all Late Pleistocene and Holocene occurrences** in order to avoid this bias.

### BIAS CORRECTION N°1: Remove all Late Pleistocene and Holocene occurrences
...
$\to$ The distributions are much more homogeneous after the correction. Similar trends can be observed at each rank, with peaks at ~15My and ~5My.
...
$\to$ ~Half of species/genera/subfamilies have only one specimen by accepted name, but it could go up to ~50 within the same species and ~200 occurrences within the same suborder. **In our model all species are upposed to have the same abundance (identical sampling rates among branches), so those huge differences will have to be mitigated.** See later for a quantification of this discrepancy and a comparison with our model's assumptions

$\implies$ Our goal now will be to correct this abundance bias.

# Mitigate the abundance bias

To quantify more precisely this bias, we will:

  1. Count the **number of occurrences** for each taxon
  2. Obtain the **stratigraphic range** of each taxa - ie. the time interval in which a taxon is observed - by combining the stratigraphic age uncertainties of its occurrences
  3. Compute the **density of occurrences** = nb of occurrences / stratigraphic range, expressed in "number of occurrences by million years"

## Compute the occurrence density
### Count occurrences by accepted name

Count the number of occurrences with the same accepted name and plot their distribution.
...
#   ggtitle("Distributions of fossil age uncertainties, by accepted rank") +
#   facet_grid(accepted_rank ~ ., scales = "free")
...
ggplot(Cetacea_occ[Cetacea_occ$accepted_rank =="species",], aes(x=-max_ma, y=accepted_name)) +
  ...
  labs(colour="Nb of occurrences") +
  theme(panel.grid.major.y = element_blank(),
        axis.text.y=element_text(size=2)) +
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=my_breaks, labels=my_breaks) + 
  ggtitle("Distributions of fossil age uncertainties for all species") +
  facet_grid(accepted_rank ~ ., scales = "free")
...
ggplot(Cetacea_occ[Cetacea_occ$accepted_rank =="genus",], aes(x=-max_ma, y=accepted_name)) +
  ...
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=my_breaks, labels=my_breaks) +
  ggtitle("Distributions of fossil age uncertainties for all genera") +
  facet_grid(accepted_rank ~ ., scales = "free")
...
#   theme(panel.grid.major.y = element_blank()) +
#   scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=my_breaks, labels=my_breaks) + 
#   ggtitle("Distributions of fossil age uncertainties (families)") +
#   facet_grid(accepted_rank ~ ., scales = "free")
```

$\to$ Some occurrences have too much age uncertainty (unrelated to taxon longevity in extreme cases like the one spanning 60My), they risk to artificially increase species duration.

### BIAS CORRECTION N°2: Exclude occurrences with highly uncertain dating (> 10My)

```{r, fig.asp=0.4, fig.width=10}
# Get the duration of the uncertainty intervals
Cetacea_occ$age_uncertainty <- Cetacea_occ$max_ma-Cetacea_occ$min_ma

ggplot(Cetacea_occ, aes(x=age_uncertainty, fill=accepted_rank)) +
  geom_histogram(binwidth = 1) +
  scale_x_continuous(name = "Age uncertainty (My)") +
  ...
  ggtitle("Distribution of occurrences' age uncertainty and removal limit (dashed line)")
```
...
data.frame(all_ranges = dim(Cetacea_occ)[1],
           smaller_10My = dim(Cetacea_occ[Cetacea_occ$age_uncertainty<10,])[1],
           removed = dim(Cetacea_occ)[1]-dim(Cetacea_occ[Cetacea_occ$age_uncertainty<10,])[1], row.names = "Number of occurrences")
```

```{r fig.height=6}
# ggplot(Cetacea_occ, aes(x=age_runif, y=age_uncertainty)) +
#   geom_point(alpha=0.05) +
#   scale_x_continuous(name = "Uniformly drawn age (My)") +
#   scale_y_continuous(name = "Age uncertainty (My)") +
#   ggtitle("Distribution of occurrences' age uncertainty")

ggplot(Cetacea_occ[Cetacea_occ$age_uncertainty != 63.412,], aes(x=age_runif, y=age_uncertainty)) +
  geom_point(alpha=0.05) +
  scale_x_continuous(name = "Uniformly drawn age (My)") +
  scale_y_continuous(name = "Age uncertainty (My)") +
...
# ggplot(Cetacea_occ[Cetacea_occ$age_uncertainty<10,], aes(x=age_runif, y=age_uncertainty)) +
#   geom_point(alpha=0.05) +
#   scale_x_continuous(name = "Uniformly drawn age (My)") +
#   scale_y_continuous(name = "Age uncertainty (My)") +
#   ggtitle("Distribution of occurrences' age uncertainty")
...
hist(-Cetacea_occ[Cetacea_occ$age_uncertainty<10,]$age_runif, breaks = 50, col="blue", probability=T, density=T, add=T)
```
...
# ggplot(Cetacea_occ[!(Cetacea_occ$accepted_rank %in% c("species", "genus", "family")) & Cetacea_occ$age_uncertainty<10,], aes(x=-max_ma, y=accepted_name)) +
#   geom_segment(aes(x = -max_ma, y = reorder(accepted_name, -age_mean), xend = -min_ma, yend=reorder(accepted_name, -age_mean), color=accepted_name_count), arrow=arrow(ends = "both", angle=90, length = unit(1, "mm")), alpha=0.5) +
...
ggplot(Cetacea_occ[Cetacea_occ$accepted_rank =="species" & Cetacea_occ$age_uncertainty<10,], aes(x=-max_ma, y=accepted_name)) +
  geom_segment(aes(x = -max_ma, y = reorder(accepted_name, -age_mean), xend = -min_ma, yend=reorder(accepted_name, -age_mean), color=accepted_name_count), arrow=arrow(ends = "both", angle=90, length = unit(1, "mm")), alpha=0.5) +
  geom_point(aes(x=-age_runif), col="red", cex=1, alpha=0.5) +
  scale_x_continuous(name = "Time (My)") +
  scale_y_discrete(name = "Age uncertainties") +
  labs(colour="Accepted names count") +
  theme(panel.grid.major.y = element_blank(),
        axis.text.y=element_text(size=2)) +
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=my_breaks, labels=my_breaks) + 
  ggtitle("Distributions of fossil age uncertainties (species)") +
  facet_grid(accepted_rank ~ ., scales = "free")

ggplot(Cetacea_occ[Cetacea_occ$accepted_rank =="genus" & Cetacea_occ$age_uncertainty<10,], aes(x=-max_ma, y=accepted_name)) +
  geom_segment(aes(x = -max_ma, y = reorder(accepted_name, -age_mean), xend = -min_ma, yend=reorder(accepted_name, -age_mean), color=accepted_name_count), arrow=arrow(ends = "both", angle=90, length = unit(1, "mm")), alpha=0.5) +
  geom_point(aes(x=-age_runif), col="red", cex=1, alpha=0.5) +
  scale_x_continuous(name = "Time (My)") +
  scale_y_discrete(name = "Genus") +
  ...
  theme(panel.grid.major.y = element_blank(),
        axis.text.y=element_text(size=6)) +
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=my_breaks, labels=my_breaks) + 
  ggtitle("Distributions of fossil age uncertainties (genera)") +
  facet_grid(accepted_rank ~ ., scales = "free")

# ggplot(Cetacea_occ[Cetacea_occ$accepted_rank =="family" & Cetacea_occ$age_uncertainty<10,], aes(x=-max_ma, y=accepted_name)) +
#   geom_segment(aes(x = -max_ma, y = reorder(accepted_name, -age_mean), xend = -min_ma, yend=reorder(accepted_name, -age_mean), color=accepted_name_count), arrow=arrow(ends = "both", angle=90, length = unit(1, "mm")), alpha=0.5) +
#   geom_point(aes(x=-age_runif), col="red", cex=1, alpha=0.5) +
#   scale_x_continuous(name = "Time (My)") +
#   scale_y_discrete(name = "Genus") +
...
$\to$ Some taxa have several occurrences with distinct age uncertainty intervals, **let's combine them into a unique stratigraphic range**.

### Stratigraphic ranges = unique time interval combining age uncertainties of occurrences of the same taxon (excluding the most uncertain ones)

```{r, fig.asp=0.5, fig.width=10, warning=FALSE}
# Get the minimun of the minimal ages for each accepted name
Cetacea_occ$min_min_ma <- apply(Cetacea_occ, 1, function(X){min(Cetacea_occ[Cetacea_occ$accepted_name==X["accepted_name"] & Cetacea_occ$age_uncertainty<10,]$min_ma)})
# Get the maximum of the maximal ages for each accepted name
Cetacea_occ$max_max_ma <- apply(Cetacea_occ, 1, function(X){max(Cetacea_occ[Cetacea_occ$accepted_name==X["accepted_name"] & Cetacea_occ$age_uncertainty<10,]$max_ma)})
# Get the stratigraphic range
Cetacea_occ$stratigraphic_range <- mapply(function(min_min_age, max_max_age){paste("[",-max_max_age,", ",-min_min_age,"]", sep = "")}, Cetacea_occ$min_min_ma, Cetacea_occ$max_max_ma)
# Get the stratigraphic range midpoint
Cetacea_occ$stratigraphic_range_midpoint <- (Cetacea_occ$min_min_ma + Cetacea_occ$max_max_ma)/2
# Get the duration of the combined time intervals
Cetacea_occ$stratigraphic_range_duration <- Cetacea_occ$max_max_ma-Cetacea_occ$min_min_ma

# ggplot(Cetacea_occ[!(Cetacea_occ$accepted_rank %in% c("species", "genus", "family")) & Cetacea_occ$age_uncertainty<10,], aes(x=-max_ma, y=accepted_name)) +
#   geom_segment(aes(x = -max_max_ma, y = reorder(accepted_name, -stratigraphic_range_midpoint), xend = -min_min_ma, yend=reorder(accepted_name, -stratigraphic_range_midpoint), color=accepted_name_count), arrow=arrow(ends = "both", angle=90, length = unit(1, "mm")), alpha=0.5) +
#   geom_point(aes(x=-age_runif), col="red", cex=1, alpha=0.5) +
#   scale_x_continuous(name = "Time (My)") +
#   scale_y_discrete(name = "Accepted ranks") +
#   labs(colour="Accepted names count") +
#   theme(panel.grid.major.y = element_blank()) +
#   scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=my_breaks, labels=my_breaks) + 
#   ggtitle("Distributions of fossil combined age uncertainties, by accepted rank") +
#   facet_grid(accepted_rank ~ ., scales = "free")
...
ggplot(Cetacea_occ[Cetacea_occ$accepted_rank =="species" & Cetacea_occ$age_uncertainty<10,], aes(x=-max_ma, y=accepted_name)) +
  ...
  theme(panel.grid.major.y = element_blank(),
        axis.text.y=element_text(size=2)) +
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=my_breaks, labels=my_breaks) + 
  ggtitle("Distributions of fossil combined age uncertainties (species)") +
  facet_grid(accepted_rank ~ ., scales = "free")
...
ggplot(Cetacea_occ[Cetacea_occ$accepted_rank =="genus" & Cetacea_occ$age_uncertainty<10,], aes(x=-max_ma, y=accepted_name)) +
  ...
  theme(panel.grid.major.y = element_blank(),
        axis.text.y=element_text(size=6)) +
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=my_breaks, labels=my_breaks) + 
  ggtitle("Distributions of fossil combined age uncertainties (genera)") +
  facet_grid(accepted_rank ~ ., scales = "free")
...
#   theme(panel.grid.major.y = element_blank()) +
#   scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=my_breaks, labels=my_breaks) + 
#   ggtitle("Distributions of fossil combined age uncertainties (families)") +
#   facet_grid(accepted_rank ~ ., scales = "free")
```

### Density distributions

Plot the density of occurrences (= number of occurrences by unit of time, in the stratigraphic interval of a taxon) for each taxonomic rank.

```{r, fig.asp=0.5, fig.width=11}
# Get the density of occurrences in the intervals
Cetacea_occ$occurrence_density <- Cetacea_occ$accepted_name_count/Cetacea_occ$age_uncertainty

ggplot(Cetacea_occ[!(Cetacea_occ$accepted_rank %in% c("subspecies", "infraorder")) & Cetacea_occ$age_uncertainty<10,], aes(x=occurrence_density)) +
  geom_histogram(binwidth=0.1) +
...
$\to$ Density logically increases with taxa ranks, but the distributions for species and genera dramatically spread over two orders of magnitude.

### Correlation between occurrence density and age
...
Let's compare the occurrence densities computed with the initial time ranges and the *combined densities* computed with the combined stratigraphic ranges.

```{r}
conditions <- Cetacea_occ$accepted_rank =="species" & Cetacea_occ$age_uncertainty<10

# Get the stratigraphic age uncertainty interval
Cetacea_occ$age_uncertainty_interval <- mapply(function(min_age, max_age){paste("[",-max_age,", ",-min_age,"]", sep = "")}, Cetacea_occ$min_ma, Cetacea_occ$max_ma)
...
age_uncertainty_interval_table <- table(Cetacea_occ[conditions,]$age_uncertainty_interval)

interval_mean_age <- sapply(names(age_uncertainty_interval_table), function(x){Cetacea_occ[conditions & Cetacea_occ$age_uncertainty_interval==x,]$age_mean[1]})

interval_density <- sapply(names(age_uncertainty_interval_table), function(x){Cetacea_occ[conditions & Cetacea_occ$age_uncertainty_interval==x,]$occurrence_density[1]})
...
ggplot(df_density, aes(x=x, y=y)) + 
  ...
  scale_x_continuous(name = "Time (My)") +
  scale_y_continuous(name = "Occurrence density") +
  ggtitle("Correlation between density and age") +
  geom_text(x = 35, y = 25, label = lm_eqn(df_density), parse = TRUE)

# Keep only one point by identical stratigraphic range
stratigraphic_range_table <- table(Cetacea_occ[conditions,]$stratigraphic_range)

stratigraphic_range_mean_age <- sapply(names(stratigraphic_range_table), function(x){Cetacea_occ[conditions & Cetacea_occ$stratigraphic_range==x,]$stratigraphic_range_midpoint[1]})

Cetacea_occ$occurrence_combined_density <- Cetacea_occ$accepted_name_count/Cetacea_occ$stratigraphic_range_duration

stratigraphic_range_density <- sapply(names(stratigraphic_range_table), function(x){Cetacea_occ[conditions & Cetacea_occ$stratigraphic_range==x,]$occurrence_combined_density[1]})

stratigraphic_range_df_density <- data.frame(x=stratigraphic_range_mean_age,
                                             y=stratigraphic_range_density)

ggplot(stratigraphic_range_df_density, aes(x=x, y=y)) + 
  geom_point(alpha=0.5) +
  geom_smooth(formula = y~x, method=lm) + 
  scale_x_continuous(name = "Time (My)") +
  scale_y_continuous(name = "Combined occurrence density") +
  ggtitle("Correlation between combined density and age") +
  geom_text(x = 15, y = 6, label = lm_eqn(stratigraphic_range_df_density), parse = TRUE)
```

$\to$ The density based on combined ranges seems less time-dependent than the density based on initial time ranges. We will therefore use the combined density for our corrections.

## BIAS CORRECTION N°3: Subsample occurrences by homogenizing the combined occurrence density

**Subsampling by homogenization** = reducing the discrepancy between high and low occurrence densities among taxa by subsampling preferentially the densest ones.
...
Cetacea_occ_sampled$stratigraphic_range_duration <- Cetacea_occ_sampled$max_max_ma-Cetacea_occ_sampled$min_min_ma
...
Cetacea_occ_sampled$occurrence_combined_density <- Cetacea_occ_sampled$accepted_name_count/Cetacea_occ_sampled$stratigraphic_range_duration
...
$\to$ Subsampling by homogenization successfully reduces the density span from 2 to 1 order of magnitude.
...
# ggplot(Cetacea_occ[Cetacea_occ$accepted_rank =="species" & Cetacea_occ$age_uncertainty<10,], aes(x=-max_ma, y=accepted_name)) +
#   geom_segment(aes(x = -max_max_ma, y = reorder(accepted_name, -stratigraphic_range_midpoint), xend = -min_min_ma, yend=reorder(accepted_name, -stratigraphic_range_midpoint), color=occurrence_combined_density), arrow=arrow(ends = "both", angle=90, length = unit(1, "mm")), alpha=0.5) +
...
#   ggtitle(paste("Distributions of species fossil age uncertainties, before correcting sampling (n = ", dim(Cetacea_occ[Cetacea_occ$accepted_rank =="species" & Cetacea_occ$age_uncertainty<10,])[1], ")", sep=""))
...
ggplot(Cetacea_occ_sampled, aes(x=-max_ma, y=accepted_name)) +
  ...
  theme(axis.text.y=element_blank(),
        ...
        panel.grid.major.y = element_blank()) +
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=breaks, labels=breaks) + 
  ggtitle(paste("Distributions of species fossil age uncertainties, after subsampling by homogenization (n = ", dim(Cetacea_occ_sampled)[1], ")", sep=""))
```
...
ggplot(Cetacea_occ_sampled, aes(x=-age_runif)) +
  ...
  scale_fill_discrete(name="Accepted rank") +
  theme(panel.grid.major.y = element_blank()) +
  ggtitle(paste("Evolution of the species occurrence number through time, after subsampling by homogenization (n = ", dim(Cetacea_occ_sampled)[1], ")", sep=""))
```
...
$\to$ We get the new species occurrence distribution after subsampling, that could be used for doing inference with the occurrence birth-death model.

### Compare with a Poisson sampling process

In order to check if the data fit our assumptions of constant-fossilization-rate Poisson sampling we compare the observed occurrences distributions with the expected ones. Specifically, we will look at the number of taxa represented by 1, 2, 3, ... occurrences and the one that we would expect for a given distribution of stratigraphic ranges (seen here as a proxy for species duration).

```{r, fig.asp=0.5, fig.width=10}
conditions <- !(Cetacea_occ$accepted_rank %in% c("subspecies", "infraorder")) & Cetacea_occ$age_uncertainty<10

ggplot(Cetacea_occ[conditions,], aes(x=stratigraphic_range_duration, fill=accepted_rank)) +
  geom_histogram(binwidth = 1) + 
  #geom_point() +
  ...
  theme(panel.grid.major.x = element_blank(),
        legend.position="bottom") +
  ggtitle("Observed distributions of stratigraphic ranges") +
  facet_grid(. ~ accepted_rank, scales = "free")
...
So in order to have the absolute probability of observing $N_0 = n$ occurrences we have to integrate over the full distribution of age uncertainties $t$, called $f(t)$:
...
```{r}
range_density <- approxfun(density(Cetacea_occ[Cetacea_occ$accepted_rank=="species",]$stratigraphic_range_duration[is.finite(Cetacea_occ[Cetacea_occ$accepted_rank=="species",]$stratigraphic_range_duration)]))

hist(Cetacea_occ[Cetacea_occ$accepted_rank=="species",]$stratigraphic_range_duration, probability = T, breaks = 20, main="Density approximation of the empirical stratigraphic range distribution", xlab = "Age range (My)")
lines(1:100/5, range_density(1:100/5))
```

Then integrate and plot the expected distribution for a given omega:

```{r fig.asp=0.3, fig.width=11}
expected_distribution <- function(n, omega, stratigraphic_range_duration){
  range_density <- approxfun(density(stratigraphic_range_duration[is.finite(stratigraphic_range_duration)]))
  g <- function(t){dpois(n, omega*t)*range_density(t)}
  return (integrate(g, lower = 1, upper = max(stratigraphic_range_duration)))
}

omega <- 0.1

dens <- sapply(0:20, function(n){expected_distribution(n, omega, Cetacea_occ[Cetacea_occ$accepted_rank=="species",]$stratigraphic_range_duration)$value})
...
dens <- sapply(1:max(accepted_name_count_species), function(n){ expected_distribution(n, omega, Cetacea_occ[Cetacea_occ$accepted_rank=="species",]$stratigraphic_range_duration)$value })
...
dens_sampled <- sapply(1:max(accepted_name_count_species_sampled), function(n){
     expected_distribution(n, omega, Cetacea_occ_sampled[Cetacea_occ_sampled$accepted_rank=="species",]$stratigraphic_range_duration)$value
    })
...
## BIAS CORRECTION N°3bis: Subsample occurrences by aggregating similarly identified occurrences in each collection

**Subsampling by aggregation** = aggregate all the occurrences of a given taxon according to a given criterion (collection, geological formation, country, geological plate).

## Wrong track: Aggregate similarly identified occurrences in each collection
...
dens_aggreg <- sapply(1:max(accepted_name_count_species_aggreg), function(n){
     expected_distribution(n, omega, Cetacea_occ_aggreg[Cetacea_occ_aggreg$accepted_rank=="species",]$stratigraphic_range_duration)$value
    })
...
$\to$ The initial bias is mostly corrected.
...
$\to$ Comparing with the initial occurrences distribution and with the distribution after our first sub-sampling it appears that both methods lead to very similar distributions. This comforts us about the robustness of those approaches.
...
## Wrong track: Aggregate occurrences without formation by country + early interval
...
$\to$ The correspondence is still good, except for two taxa:
...
```{r, warning=FALSE}
Cetacea_occ_aggreg$age_uncertainty <- Cetacea_occ_aggreg$max_ma-Cetacea_occ_aggreg$min_ma
Cetacea_occ_aggreg$min_min_ma <- apply(Cetacea_occ_aggreg, 1, function(X){min(Cetacea_occ_aggreg[Cetacea_occ_aggreg$age_uncertainty<10 & Cetacea_occ_aggreg$accepted_name==X["accepted_name"],]$min_ma)})
Cetacea_occ_aggreg$max_max_ma <- apply(Cetacea_occ_aggreg, 1, function(X){max(Cetacea_occ_aggreg[Cetacea_occ_aggreg$age_uncertainty<10 & Cetacea_occ_aggreg$accepted_name==X["accepted_name"],]$max_ma)})
Cetacea_occ_aggreg$stratigraphic_range_midpoint <- (Cetacea_occ_aggreg$min_min_ma + Cetacea_occ_aggreg$max_max_ma)/2
Cetacea_occ_aggreg$stratigraphic_range_duration <- Cetacea_occ_aggreg$max_max_ma-Cetacea_occ_aggreg$min_min_ma
Cetacea_occ_aggreg$accepted_name_count <- sapply(Cetacea_occ_aggreg$accepted_name, function(x){table(Cetacea_occ_aggreg$accepted_name)[x]})
Cetacea_occ_aggreg$occurrence_combined_density <- Cetacea_occ_aggreg$accepted_name_count/Cetacea_occ_aggreg$stratigraphic_range_duration
```

```{r fig.asp=0.5, fig.width=10, message=FALSE}
breaks <- c(0.125,0.25,0.5,1,2,4,8,16)

p_before <- ggplot(Cetacea_occ[Cetacea_occ$accepted_rank =="species" & Cetacea_occ$age_uncertainty<10,], aes(x=-max_ma, y=accepted_name)) +
  geom_segment(aes(x = -max_max_ma, y = reorder(accepted_name, -stratigraphic_range_midpoint), xend = -min_min_ma, yend=reorder(accepted_name, -stratigraphic_range_midpoint), color=occurrence_combined_density), arrow=arrow(ends = "both", angle=90, length = unit(1, "mm")), alpha=0.5) +
  geom_point(aes(x=-age_runif), col="red", cex=0.5, alpha=0.5) +
  scale_x_continuous(name = "Time (My)") +
  scale_y_discrete(name = "Species") +
  ...
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=breaks, labels=breaks) + 
  ggtitle(paste("Distributions of species fossil age uncertainties, before subsampling (n = ", dim(Cetacea_occ[Cetacea_occ$accepted_rank =="species" & Cetacea_occ$age_uncertainty<10,])[1], ")", sep=""))
p_before

ggsave("../Images/Range_distributions_before_aggregating.svg", plot = p_before, device = "svg", bg = "transparent")

p_after <- ggplot(Cetacea_occ_aggreg[Cetacea_occ_aggreg$accepted_rank =="species" & Cetacea_occ_aggreg$age_uncertainty<10,], aes(x=-max_ma, y=accepted_name)) +
  geom_segment(aes(x = -max_max_ma, y = reorder(accepted_name, -stratigraphic_range_midpoint), xend = -min_min_ma, yend=reorder(accepted_name, -stratigraphic_range_midpoint), color=occurrence_combined_density), arrow=arrow(ends = "both", angle=90, length = unit(1, "mm")), alpha=0.5) +
  geom_point(aes(x=-age_runif), col="red", cex=0.5, alpha=0.5) +
  scale_x_continuous(name = "Time (My)") +
  scale_y_discrete(name = "Species") +
  ...
  theme(axis.text.y=element_blank(),
        ...
        legend.box.background = element_rect(fill = "transparent", color = NA)) +
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=breaks, labels=breaks) + 
  ggtitle(paste("Distributions of species fossil age uncertainties, after subsampling by aggregation (n = ", dim(Cetacea_occ_sampled)[1], ")", sep=""))
p_after
...
Replacing countries by geological plates seems to make more sens from a palaeontological perspective, so let's try it.
...
$\to$ The family distributions vary a bit after subsampling, but because of the limited number of species by family the fact that we corrected the oversampling of some species could have a disproportionate effect.
...
# Genera for which the geological formation is missing
# Browse all geoplates
geoplates <- levels(droplevels(Cetacea_occ[Cetacea_occ$formation=="",]$geoplate))
for (gp in geoplates){
  Cetacea_occ_gp <- Cetacea_occ[Cetacea_occ$formation=="" & Cetacea_occ$geoplate==gp,]
...
dens <- sapply(1:max(genus_count), function(n){ expected_distribution(n, omega, Cetacea_occ[Cetacea_occ$accepted_rank %in% c("species", "genus"),]$stratigraphic_range_duration)$value })
...
dens_aggreg <- sapply(1:max(genus_count_aggreg), function(n){
     expected_distribution(n, omega, Cetacea_occ_aggreg_gen[Cetacea_occ_aggreg_gen$accepted_rank %in% c("species", "genus"),]$stratigraphic_range_duration)$value
    })
...
$\to$ The correspondence is much less improved than with species aggregation because some genera have a lot of occurrences due to their high number of species:
...
p1 + annotation_custom(ggplotGrob(p3)) + annotation_custom(ggplotGrob(p2)) + ggtitle("Initial occurrences distribution (red) and comparison after sub-sampling (blue) or aggregating in geoplates (green)")
```

$\to$ Delimiting by geological plates (+ age) instead of countries (+ age) leads to similar distributions, with a bit more occurrences.
...
Several biases affect the cetacean fossil record:

  - BIAS 1: Very recent samples may have been dated with a more precise method and contain much more fossils, so they should be removed or treated separately
  - BIAS 2: Some occurrences show a highly uncertain dating (> 10My), they are excluded when combining the uncertainty intervals into a unique stratigraphic range per taxon
  - BIAS 3: Some taxa and geological periods seems to have been oversampled, due to variation in species abundance, geological factors or human unequal sampling, 
    1. It seems possible to adequately reduce the abundance bias by subsampling the richest taxa, based on a metrics of occurrence density (number of occurrences / stratigraphic range). Using combined stratigraphic ranges, instead of age uncertainty intervals, appears to be more robust
    2. An alternative sampling method consisting in aggregating the occurrences of the same taxa found in the same geological formation (or the same geoplate + early stratigraphic interval if the geological formation is not indicated) seems to work equally well, and is more paleontologically relevant
