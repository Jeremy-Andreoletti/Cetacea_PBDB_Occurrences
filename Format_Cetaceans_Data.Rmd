...
$\to$ Numerous occurrences seem to have the same age interval so in order to avoid clusters let's draw them uniformly in their stratigraphic range rather than taking the midpoint.
...
$\to$ Apparent huge cluster of occurrences in recent times, with very precise dating = Artefact due to the "Pull of the Recent" effect ? 

$\implies$ We decided to **remove all Late Pleistocene and Holocene occurrences** (thus setting the $\omega$-sampling to 0) in order to avoid this bias.
...
ggplot(Cetacea_occ[Cetacea_occ$accepted_rank =="species",], aes(x=-max_ma, y=accepted_name)) +
  ...
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=my_breaks, labels=my_breaks) + 
  ggtitle("Distributions of fossil age ranges for all species") +
  facet_grid(accepted_rank ~ ., scales = "free")
...
ggplot(Cetacea_occ[Cetacea_occ$accepted_rank =="genus",], aes(x=-max_ma, y=accepted_name)) +
  ...
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=my_breaks, labels=my_breaks) +
  ggtitle("Distributions of fossil age ranges for all genera") +
  facet_grid(accepted_rank ~ ., scales = "free")
...
$\to$ Some occurrences have too much age uncertainty (unrelated to species longevity in extreme cases like the one spanning 60My), they risk to artificially increase species durations.
...
ggplot(Cetacea_occ, aes(x=age_range, fill=accepted_rank)) +
  ...
  ggtitle("Distribution of occurrences' age uncertainty range and removal limit (dashed line)")
```
...
ggplot(Cetacea_occ[!(Cetacea_occ$accepted_rank %in% c("subspecies", "infraorder")) & Cetacea_occ$age_range<10,], aes(x=occurrence_density)) +
  geom_histogram(binwidth=0.1) +
...
$\to$ The density based on combined ranges seems less time-dependant than the density based on initial time ranges. We will therefore use the combined density for our corrections.
...
ggplot(Cetacea_occ_sampled, aes(x=-max_ma, y=accepted_name)) +
  ...
  theme(axis.text.y=element_blank(),
        ...
        panel.grid.major.y = element_blank()) +
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=breaks, labels=breaks) + 
  ggtitle(paste("Distributions of species fossil age ranges, after subsampling by homogenization (n = ", dim(Cetacea_occ_sampled)[1], ")", sep=""))
```
...
$\to$ We get the new species occurrence repartition after subsampling, that could be used for doing inference with the occurrence birth-death model.
...
p_before <- ggplot(Cetacea_occ[Cetacea_occ$accepted_rank =="species" & Cetacea_occ$age_range<10,], aes(x=-max_ma, y=accepted_name)) +
  ...
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=breaks, labels=breaks) + 
  ggtitle(paste("Distributions of species fossil age ranges, before subsampling (n = ", dim(Cetacea_occ[Cetacea_occ$accepted_rank =="species" & Cetacea_occ$age_range<10,])[1], ")", sep=""))
p_before
...
p_after <- ggplot(Cetacea_occ_aggreg[Cetacea_occ_aggreg$accepted_rank =="species" & Cetacea_occ_aggreg$age_range<10,], aes(x=-max_ma, y=accepted_name)) +
  ...
  theme(axis.text.y=element_blank(),
        ...
        legend.box.background = element_rect(fill = "transparent", color = NA)) +
  scale_color_gradientn(colours = c("darkblue", "forestgreen", "black"), trans="log", breaks=breaks, labels=breaks) + 
  ggtitle(paste("Distributions of species fossil age ranges, after subsampling by aggregation (n = ", dim(Cetacea_occ_sampled)[1], ")", sep=""))
p_after
